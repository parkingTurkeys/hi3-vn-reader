for ($n = 0; $n -lt 27; $n++) {
    ...
    for ($i = 0; $i -lt $image_urls.length; $i++) {
        ...
        #$image_urls[$i] = $image_urls[$i] -replace ' ', '%20'
        $image_urls[$i] = "https://act-webstatic.mihoyo.com/event_bh3_com/avg-anti-entropy/static_CN/resources/sound/" + $image_urls[$i]
    }
    # echo $image_urls
    ...
    for ($i = 0; $i -lt $image_urls.length; $i++) {
        ...
        # echo $file_name
        try {(new-object System.Net.WebClient).DownloadFile( $image_urls[$i], "D:\projects\hi3-vn-reader\bgm\$file_name") } catch { }
    }
} 
